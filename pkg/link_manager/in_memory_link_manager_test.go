...
var _ = Describe("In-memory link manager tests", func() {
	...
	It("should update a link", func() {
		...
		r3 := om.GetLinksRequest{Username: "wrlou"}
	...
	It("should delete a link", func() {
		...
		// Should have 1 link
		r2 := om.GetLinksRequest{Username: "wrlou"}
		res, err := linkManager.GetLinks(r2)
		Ω(err).Should(BeNil())
		Ω(res.Links).Should(HaveLen(1))

		// Delete the link
		err = linkManager.DeleteLink("wrlou", r.Url)
		Ω(err).Should(BeNil())
	...
	It("should update link status when receiving OnLinkChecked() calls", func() {
		...
		linkCheckSink.OnLinkChecked("wrlou", r.Url, om.LinkStatusValid)
		...
		linkCheckSink.OnLinkChecked("wrlou", r.Url, om.LinkStatusInvalid)
