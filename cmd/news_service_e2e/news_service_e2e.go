...
import (
	...
	nmc "github.com/wrlou/delinkcious/pkg/news_manager_client"
...
func main() {
	...
	sender.OnLinkAdded("wrlou", &om.Link{Url: "http://example.org", Title: "Example"})
	time.Sleep(time.Second * 1)

	fmt.Println("Fetching news...")
	res, err := cli.GetNews(om.GetNewsRequest{Username: "wrlou"})
	Check(err)
